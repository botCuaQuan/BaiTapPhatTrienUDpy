...
// Nếu frontend chạy cùng domain với backend -> để trống.
// Nếu chạy khác (VD: http://localhost:5500) và backend ở http://localhost:8000 -> set API_BASE = "http://localhost:8000"
const API_BASE = "";
...
async function renderDashboard(statusMsg = "") {
    ...
    appEl.innerHTML = `
        ...
        <div class="card">
            ...
            &nbsp;
            <button class="btn btn-secondary" id="stop-all-coins">⛔ Chỉ dừng toàn bộ COIN</button>
        </div>
    `;
...
async function init() {
    try {
        ...
        appEl.innerHTML = `
            ...
            <p>Kiểm tra xem backend FastAPI (PHẦN 1) đã chạy chưa nhé.</p>
        `;
    }
}
