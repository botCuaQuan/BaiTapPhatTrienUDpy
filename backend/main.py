...
from trading_bot_lib import BotManager  # dùng lại logic bot của bạn
...
# Cho phép web/app mobile ở domain khác gọi API
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],        # sau này có domain cụ thể thì siết lại
...
# =============================
# HÀM TIỆN ÍCH HTML (WEB BACKEND ĐƠN GIẢN)
# =============================
...
def is_account_configured() -> bool:
    return (
        app.state.api_key is not None
        and app.state.api_secret is not None
        and app.state.bot_manager is not None
    )
...
async def api_account_status():
    return {"configured": is_account_configured()}
